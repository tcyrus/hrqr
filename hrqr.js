...
const letterDatabase = require('./HRQRLetterdatabase.js');
const globalStates = {
...
// start canvas
function drawHRQR(messageContent, size, color, dotColor, outline, background) {
	globalStates.width = globalStates.height = (size === undefined) ? 8 : size;

	globalStates.color = (color === undefined) ? '#000' : color;

	globalStates.dotColor = (dotColor === undefined) ? globalStates.color : dotColor;

	globalStates.left = globalStates.top = (outline === undefined) ? 20 : outline;

	globalStates.background = (background === undefined) ? '#FFF' : background;

	globalStates.text = messageContent;

	var textCheck = (globalStates.text === '') ? '0' : makeThree(itob62(crc16(globalStates.text)));

	// generate text with crc32 checksum
	globalStates.text += ` & ${textCheck}`;
	...
	globalStates.rowCounter = globalStates.lineCounter = globalStates.line = 0;
	...
	while (globalStates.textTrimLength < amountOfLetters) {
		globalStates.text += randomLetter();
		globalStates.textTrimLength++;
	}

	var rowColSize = (4 * globalStates.colums * globalStates.width) + (2 * globalStates.left) - globalStates.width;

	globalStates.htmlText = `<svg xmlns="http://www.w3.org/2000/svg" width="${rowColSize}" height="${rowColSize}">`;
	globalStates.htmlText += `\n\t<rect x="0" y="0" width="${rowColSize}" height="${rowColSize}" fill="${globalStates.background}"/>`;

	writeLetters();

	globalStates.htmlText += '\n</svg>';

	return globalStates.htmlText;
}
...
function writeLetters() {
	...
	for (var i = 0; i < globalStates.text.length; i++) {
		...
		if (globalStates.countingLine >= globalStates.colums) {
			globalStates.line++;
			globalStates.rowCounter = globalStates.countingLine = 0;
		}
			...
			if (letter != undefined && globalStates.text[i+1] != undefined) {
				if (globalStates.countingLine >= globalStates.colums - 1) {
					...
					if (i + globalStates.colums <= globalStates.text.length) {
						// renderConnector(letter, globalStates.text[i+globalStates.colums], size, (letterSize(i+1) === 'small'), true);
						renderConnector(letter, letter, size, (letterSize(i+1) === 'small'), true);
					}
				} else {
					// console.log('x');
					renderConnector(letter, globalStates.text[i+1], size, (letterSize(i+1) === 'small'), false);
				}
			}
		}
		...
		if (letter in globalStates.database) {
			if (letter !== ' ') {
				if (!('small' in letterDatabase[letter])) size = 'big';
				...
				globalStates.countingLine += countsize;
			}

			for (var w = 0; w < sideA.length; w++) {
				if (sideA[w] === 0) renderDot(4, w);
				if (sideB[w] === 0) renderDot(2, w);
			}
		}
	}
}
...
function renderConnector(letterA, letterB, size, nextSize, possition) {
	if (letterA in globalStates.database && letterB in globalStates.database) {
		var grade = [],
			gradeA = globalStates.database[letterA],
			gradeB = globalStates.database[letterB];

		gradeA = gradeA[(size in gradeA) ? size : 'big'];
		gradeB = gradeB[(size in gradeB) ? size : 'big'];

		if (possition === true) {
			gradeA = gradeA.bottom;
			gradeB = gradeB.top;
		} else {
			gradeA = gradeA.right;
			gradeB = gradeB.left;
		...
		if (possition === true) renderDot(1, finalPossition, true);
		else renderDot(1, finalPossition, false);
	}
}
...
function renderDot(vertical, horizontal, possition) {
	...
	globalStates.htmlText += `\n\t<rect x="${yy}" y="${xx}" width="${globalStates.width}" height="${globalStates.height}" fill="${globalStates.dotColor}"/>`;
}
	...
	if (thisLetter in globalStates.database) {
		...
		for (var i = 0; i < shapeLength; i++) {
			if (shape[i] === 1) {
				...
				globalStates.htmlText += `\n\t<rect x="${yy}" y="${xx}" width="${globalStates.width}" height="${globalStates.height}" fill="${globalStates.color}"/>`;
			}
...
function makeThree(i) {
	if (i.length > 3) return i.substring(0, 3);
	else if (i.length === 2) return '.' + i;
	else if (i.length === 1) return '..' + i;
	else return i;
}

module.exports = drawHRQR;
